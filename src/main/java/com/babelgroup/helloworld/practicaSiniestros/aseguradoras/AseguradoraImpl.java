...
@Component
public class AseguradoraImpl implements Aseguradora {

    private static final double VALOR_RESIDUAL = 0.15;

    public double calcular(Siniestro siniestro) {
        Bien bienAfectado = siniestro.getBienAfectado();
        double valorResidual = bienAfectado.getValor() * VALOR_RESIDUAL;
        double valorDepreciado = bienAfectado.getValor();
        ...
        return valorDepreciado < valorResidual ? valorResidual : valorDepreciado;
    }
    ...
    private int calcularAniosDepreciacion(Date fechaAdquisicion, Date fechaSiniestro) {
        ...
        return Period.between(localAdquisicion, localSiniestro).getYears();
    }
